...
def evaluate_accuracy(net, loss, data_iter,GPU_mode,test=False):  
    ...
    with torch.no_grad():
        for X, y in data_iter:
            X,y=(X.cuda(),y.cuda()) if GPU_mode else (X,y)  
            y_hat=net(X) 
            l=loss(y_hat,y)
            metric.add(l.sum(), accuracy(y_hat, y), torch.tensor(y.numel()).cuda() if GPU_mode else torch.tensor(y.numel()))
            if test:
...
def train_procedure_in_each_epoch(net,tran_set,loss,optimizer,GPU_mode,local_rank):
    ...
    for X, y in tran_set:
        X,y=(X.cuda(),y.cuda()) if GPU_mode else (X,y)
        optimizer.zero_grad()
        y_hat=net(X) #It's new in PN, need points
        l=loss(y_hat,y)
        l.mean().backward()
        optimizer.step()
        ################################################
