...
#    \ V  V /    / /_  |  _|  
#     \_/\_/    /____| |_|    
#            PFN                Note: This script can run on the CPU or any GPU(s), for testing purposes only.
...
def main(local_rank, train_set, val_set, test_set, GPU_mode=True):
    ...
    if local_rank==0:
        ...
        writer.add_graph(net,input_to_model=torch.rand(1,*input_shape_for_one_event).to(local_rank) if GPU_mode else torch.rand(1,*input_shape_for_one_event))
    ...
    for epoch in range(EPOCHS):
        ...
        loss_train,acc_train=my_tools_PFN.train_procedure_in_each_epoch(net,train_set,loss,optimizer,GPU_mode,local_rank)
        loss_val,acc_val=my_tools_PFN.evaluate_accuracy(net,loss,val_set,GPU_mode,test=False)
    ...
    if local_rank==0:
        ...
        score,true_label,loss_test,acc_test=my_tools_PFN.evaluate_accuracy(net,loss,test_set,GPU_mode,test=True)
